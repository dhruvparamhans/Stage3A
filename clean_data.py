...
def mirrored(maxval, inc=16./1197):
...
def clip_and_smooth(raw, cutoff,offset=3, max_freq = 384.2390, min_freq = 384.2230, mid_freq = 384.231, resolution=16./1197):
	...
	temp = raw[cutoff+offset:]
	#max_freq = max_freq - (resolution*cutoff/1000)
	min_freq_temp = min_freq + (resolution*cutoff/1000)
	difference = max_freq - min_freq_temp
	step = difference/len(temp)
	#mid_freq_temp = (max_freq+min_freq_temp)/2
	#print mid_freq_temp
	freqs = np.arange(min_freq_temp, max_freq, step)
...
## Retrieve raw data from the folder /Absorption_24_Mars 


## Clip and Smooth data for p147 
